...
<#
.SYNOPSIS
    Intune Win32 detection script for Project Professional (ProjectProRetail).

.DESCRIPTION
    Reads HKLM:\SOFTWARE\Microsoft\Office\ClickToRun\Configuration and matches
    ProductReleaseIds -contains 'ProjectProRetail'.

.NOTES
    Script  : Detect-Project.ps1
...
param(
    ...
    [string] $LogFile = 'Project-Detection.log'
)
...
function Write-CMTraceLine {
    param(
        ...
        [string] $Component = 'Detect-Project',
    ...
    try {
        ...
        $line = '<![LOG[{0}]LOG]!><time="{1}{2}" date="{3}" component="{4}" context="{5}" type="{6}" thread="{7}" file="Detect-Project.ps1:0">' -f `
