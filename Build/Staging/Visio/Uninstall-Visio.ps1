...
<#
.SYNOPSIS
    Uninstall Visio (VisioProRetail) without affecting Office or Project.

.DESCRIPTION
    Uses visio-remove.xml with a scoped <Remove All="FALSE"> block so that
    M365 Apps and Project remain installed.

.PARAMETER ConfigurationFile
    Path to a local removal XML. Defaults to visio-remove.xml.

.PARAMETER ConfigurationURL
    HTTPS URL to a removal XML. Overrides -ConfigurationFile.
...
if (-not $PSBoundParameters.ContainsKey('ConfigurationFile') -and [string]::IsNullOrWhiteSpace($ConfigurationURL)) {
    $ConfigurationFile = Join-Path -Path $PSScriptRoot -ChildPath 'Configurations\visio-remove.xml'
}
