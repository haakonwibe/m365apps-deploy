...
<#
.SYNOPSIS
    Install Visio (VisioProRetail) on top of an existing M365 Apps install.

.DESCRIPTION
    Visio is an add-on: the base M365 Apps install must already be present,
    and the Visio channel / architecture must match it. This script enforces
    that check before launching setup.exe.

.PARAMETER ConfigurationFile
    Path to a local ODT configuration XML. Defaults to visio-base.xml.

.PARAMETER ConfigurationURL
    HTTPS URL to a configuration XML. Overrides -ConfigurationFile when set.
...
.PARAMETER SkipBaseInstallCheck
    Skip the "M365 Apps must be present first" guardrail. Advanced use only;
    installing Visio on a machine without Office is technically valid but is
    rarely what admins want.
...
if (-not $PSBoundParameters.ContainsKey('ConfigurationFile') -and [string]::IsNullOrWhiteSpace($ConfigurationURL)) {
    $ConfigurationFile = Join-Path -Path $PSScriptRoot -ChildPath 'Configurations\visio-base.xml'
}
...
try {
    ...
    if (-not $SkipBaseInstallCheck) {
        if ($null -eq $existing -or -not (Test-ProductInstalled -ProductId 'O365ProPlusRetail')) {
            throw "Visio is an add-on; M365 Apps (O365ProPlusRetail) must be installed first. Deploy Install-M365Apps.ps1 as a prerequisite in Intune, or pass -SkipBaseInstallCheck to override."
        }
        ...
        Write-ODTLog -Message ("Base Office install detected: platform={0} channel={1}. Visio install will match." -f $existing.Platform, $channelDisplay) -Component $ScriptName -LogFile $LogFile -LogPath $LogPath
    }
    ...
    #   - Channel: if XML specifies a channel that differs from the installed
    #     channel (compared on canonical names, not the registry's GUID URL),
    #     rewrite the XML Channel attribute to match. ODT refuses cross-channel
    #     installs; rewriting is more robust than asking admins to maintain a
    #     per-channel XML variant.
    #   - Architecture: if mismatched, ODT will refuse outright. We log it at
    #     severity 3 and let ODT surface the error - 32-bit Visio on 64-bit
    #     Office is never a safe auto-fix.
    if ($null -ne $existing) {
        try {
            ...
            if ($null -ne $addNode) {
                ...
                if ($channelAttr -and $installedChannelName -and ($channelAttr -ine $installedChannelName)) {
                    ...
                    $rewrittenPath = Join-Path -Path $env:TEMP -ChildPath ("visio-channel-rewrite-$(Get-Random).xml")
                ...
                if ($archAttr -and $existingArch -and $archAttr -ne $existingArch) {
                    Write-ODTLog -Message ("Architecture mismatch: existing Office is {0}-bit, Visio XML requests {1}-bit. ODT will refuse the install." -f $existingArch, $archAttr) -Severity 3 -Component $ScriptName -LogFile $LogFile -LogPath $LogPath
                }
            }
        }
        catch {
